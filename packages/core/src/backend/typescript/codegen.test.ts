...
describe("TypeScript generation - public names", () => {
  ...
  it("emits Outputs symbols even with no declared outputs (root output only)", () => {
    const code = generate(seq(lit("bet"), str("input")), { app: { id: "bet" } });
    expect(code).toContain("export interface BetOutputs {");
    expect(code).toContain("root: OutputPathType;");
    expect(code).toContain("): BetOutputs {");
  });
...
describe("TypeScript generation - params factory & kwarg wrapper", () => {
  ...
  it("emits a kwarg `<tool>` wrapper that calls the factory then execute", () => {
    const code = generate(seq(lit("greet"), str("name")), { app: { id: "greet" } });
    expect(code).toMatch(
      /export function greet\(\s*name: string,\s*runner: Runner \| null = null,\s*\): GreetOutputs \{/,
    );
