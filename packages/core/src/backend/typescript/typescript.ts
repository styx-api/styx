...
export function generateTypeScript(ctx: CodegenContext): string {
  ...
  // File outputs and mutable inputs are path-valued (OutputPathType) and drive
  // that import; stream outputs (stdout/stderr) only add string[] fields. Any
  // of the three means the Outputs interface/builder/return are emitted.
  const usesOutputPath = hasAnyOutputs(ctx) || hasMutableInputs(ctx);
  const emitOutputs = usesOutputPath || hasStreamOutputs(ctx);

  emitImports(cb, usesOutputPath);
  cb.blank();
