...
export function generateTypeScript(ctx: CodegenContext): string {
  ...
  const { namedTypes, typeDecls } = collectNamedTypes(rootType, rootName, scope, pascalCase);
  ...
  // Optional stripExtensions helper used by output ref tokens.
  ...
  emitWrapperFunction(ctx, paramsType, funcName, metaConst, cb, emitOutputs);
  cb.blank();
