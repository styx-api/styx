...
| `mrtrixDemo`          | Hand-written, the only `mrtrix` (native C++ dump) source-format fixture: exercises the frontend's output-file args, the `various` `str\|file` union, a comma-separated `int seq`, a repeatable multi-arg option (`-config`), and a positional/option id collision (`output`) that the frontend disambiguates (`output` -> `output_2`) - shapes specific to the MRtrix dump frontend.                                                                                        |
